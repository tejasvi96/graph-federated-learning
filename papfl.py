    ...
    def save_results(self,loss_values,loss_values_local,val_loss):
        if len(loss_values)!=0:
            with h5py.File('./'+self.options['algorithm']+'_global_loss.hdf5','w') as fp:
                fp.create_dataset('loss_values',data=np.array(loss_values))
                fp.close()
        if len(loss_values_local)!=0:
            with h5py.File('./'+self.options['algorithm']+'_local_loss.hdf5','w') as fp:
                fp.create_dataset('loss_values_local',data=np.array(loss_values_local))
                fp.close()
        if len(val_loss)!=0:
                ...
                for c in range(self.params['NUM_CLIENTS']):
                    if update_alpha:
                        for c2 in range(self.params['NUM_CLIENTS']):
                            ...
                            for param in self.models[c]['private'].named_parameters():
                                ...
                                diff=self.models[c]['private'].state_dict()[param[0]][:]-self.models[c2]['private'].state_dict()[param[0]][:]
                                ...
                                v+=val
                            v=v/(norm1*norm2)
                            self.alphas[c][c2]=self.alphas[c][c2] - self.options['alpha_lr']*v.item()
                    ...
                    for param in self.models[c]['personalized'].named_parameters():
                        ...
                        for c2 in range(self.params['NUM_CLIENTS']):
                            ...
                            self.models[c]['personalized'].state_dict()[param[0]][:]+=(self.models[c]['private'].state_dict()[param[0]][:]*(self.alphas[c][c2]/degree)+self.models[c2]['private'].state_dict()[param[0]][:]*((1.0-self.alphas[c][c2])/degree))
