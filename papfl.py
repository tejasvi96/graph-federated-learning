        ...
        for round_num in range(self.params['num_rounds']):
            ...
            if((round_num+1)%self.params['tau']==0):
                ...
                for c in range(self.params['NUM_CLIENTS']):
                    self.models[c]['private_copy']=copy.deepcopy(self.models[c]['private'])
            else:
                # for param in self.models[0]['private'].named_parameters():
                #   print(self.models[0]['private'].state_dict()[param[0]][:]-self.models[0]['private_copy'].state_dict()[param[0]][:])
                ...
                for c in range(self.params['NUM_CLIENTS']):
                        ...
                        for c2 in range(self.params['NUM_CLIENTS']):
                            ...
                            for param in self.models[c]['private'].named_parameters():
                                if param[1].grad is None:
                                    continue
                                grads=param[1].grad
                                diff=self.models[c]['private'].state_dict()[param[0]][:]-self.models[c2]['private_copy'].state_dict()[param[0]][:]
                                ...
                                v+=val
                            v=v/(norm1*norm2)
                            self.alphas[c][c2]=self.alphas[c][c2] - self.options['alpha_lr']*v.item()*degree
                    ...
                    for param in self.models[c]['personalized'].named_parameters():
                        ...
                        for c2 in range(self.params['NUM_CLIENTS']):
                            if c==c2:
                                continue
                            self.models[c]['personalized'].state_dict()[param[0]][:]+=(self.models[c]['private'].state_dict()[param[0]][:]*(self.alphas[c][c2]/degree)+self.models[c2]['private_copy'].state_dict()[param[0]][:]*((1.0-self.alphas[c][c2])/degree))
                    # else:
